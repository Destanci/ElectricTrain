...
if mods["space-exploration"] then
  ...
  table.insert(data.raw["technology"]["se-space-rail"].effects, {
    type = "unlock-recipe",
    recipe = "et-fluid-wagon-4"
  })
  table.insert(data.raw["technology"]["se-space-rail"].effects, {
    type = "unlock-recipe",
    recipe = "et-cargo-wagon-4"
  })
end
...
if mods['FactorioExtended-Trains'] and settings.startup['mk-train'].value then
	...
	data.raw['cargo-wagon']['cargo-wagon-4'].max_speed = 2.8
	data.raw['fluid-wagon']['fluid-wagon-2'].max_speed = 1.8
	data.raw['fluid-wagon']['fluid-wagon-3'].max_speed = 2.4
	data.raw['fluid-wagon']['fluid-wagon-4'].max_speed = 2.8
end
...
if mods['bobvehicleequipment'] then
	if settings.startup['bobmods-vehicleequipment-enablevehiclegrids'].value then
		...
		if settings.startup['mk-train'] then
			...
			data.raw['locomotive']['et-electric-locomotive-4'].equipment_grid = "bob-locomotive-3"
		end
		if settings.startup['mk-cargo'].value then
			...
			data.raw['cargo-wagon']['et-cargo-wagon-4'].equipment_grid = "bob-cargo-wagon-3"
		end
	end
end
